...
검증용 `POST /api/admin/probe/naver`는 `PROBE_ENABLED=false`로 운영 비활성화 확인을 마쳤다. 재개하려면 수집 허용 근거 검토 후 운영자가 설정·배포해야 한다. 현재 코드에서는 robots 검사 자체의 429/403/401도 당일 중단 대상으로 기록한다.

### 배포 마무리 확인

- 배포 소스 커밋: `368814d`, Cloudflare deployment `d28f52e5-3179-449e-b4f6-45f922102149`.
- 취소로 표시된 도구 호출과 달리 Cloudflare 측 배포는 이미 완료되어 있었다. 중복 배포하지 않고 실제 운영 상태를 확인했다.
- `/health` 200, DB ready, `serviceReady:false`.
- 미인증 운영 API 401, 인증된 임시 probe 호출 403 `PROBE_DISABLED`.
- 기존 사전검증 결과는 D1에 `policy_blocked`로 남아 있다. 이번 확인에서는 네이버에 새 요청을 보내지 않았다.
- probe가 활성화되어 있던 이전 임시 배포 `9fd6722c-dbd6-4f6d-ae79-bcd6a77ed04e`는 삭제했다. 현재 배포 및 D1은 유지했다.
- SSO·공급 API·크론 시크릿은 미설정 상태로 확인했다. 운영 불가능한 기능을 200으로 응답하지 않는다.
