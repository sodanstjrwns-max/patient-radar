...
export function prescribe(s: Signal): string[] {
  ...
  if (s.placeRank == null) {
    if (s.mentions > 0) out.push(`블로그·카페엔 「${t}」로 우리 글이 잡히는데 플레이스 목록엔 없음 → 플레이스 관리에서 대표 키워드·업종 소개·진료 항목(메뉴)에 「${t}」 추가, 예약 후 리뷰 요청 문구에 「${t}」 포함 → 방문자 리뷰에 단어가 쌓이게 하세요.`);
    ...
    out.push(`${where} → 통설상 최근 방문자 리뷰 수·답글률·저장 수가 플레이스 순위에 영향을 준다고 알려져 있습니다(네이버가 공개한 기준은 아님). 이번 달 ${patients}에게 리뷰 요청을 집중하고 모든 리뷰에 48시간 안에 답하세요.${revGap != null && revGap > 0 ? ` (1위 ${top.name}: 방문자 리뷰 ${revGap.toLocaleString()}건 더 많음)` : ""}`);
    if (top && top.blogReviews != null && s.selfBlogReviews != null && top.blogReviews > s.selfBlogReviews * 2) out.push(`위에 있는 ${top.name}: 블로그 리뷰 ${top.blogReviews.toLocaleString()}건(우리 ${s.selfBlogReviews.toLocaleString()}) → 블로그 기반 상대. 체험단보다 「${s.keyword}」 제목의 최근 글(최근 3개월) 편수를 맞추는 게 우선.`);
  } else if (s.placeRank <= 3) {
    out.push(`플레이스 ${s.placeRank}위 유지 중${s.blockSize ? `(블록 ${s.blockSize}개)` : ""} → 주 1회 플레이스 소식 게시, 리뷰 응답 유지. 순위가 내려가면 경보로 알려드립니다.`);
  }
  if (s.blogSection && s.mentions === 0 && s.placeRank != null) out.push(`통합검색 블로그 영역에 우리 글이 없음 → 「${s.keyword}」를 제목에 넣은 글 2편(원장 설명 1 + 사례 1). 플레이스만 있고 블로그가 비면 검색 결과에서 우리 이름을 만나는 자리가 하나뿐입니다.`);
  if (s.googleRank === null) out.push(`구글 비즈니스 프로필에서 「${t}」로 안 잡힘 → 프로필의 카테고리·서비스 항목에 「${t}」 추가, 「${t}」 관련 업데이트 게시 1회.`);
