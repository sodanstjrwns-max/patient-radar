...
export function prescribe(s: Signal): string[] {
  ...
  if (s.placeRank == null) {
    ...
    out.push(`${s.placeRank}위는 첫 화면(3위) 밖 → 최근 30일 방문자 리뷰 수·리뷰 응답률·저장 수가 순위를 가릅니다. 이번 달 ${patients}에게 리뷰 요청을 집중하고 모든 리뷰에 48시간 안에 답하세요.${revGap != null && revGap > 0 ? ` (1위 ${top.name}: 방문자 리뷰 ${revGap.toLocaleString()}건 더 많음)` : ""}`);
