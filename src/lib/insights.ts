...
export function buildInsights(d: DashboardData, hospitalName: string, keyTreatments: string[]): Insight[] {
  ...
  if (o && np) {
    ...
    if (missingKey) out.push({ tone: "bad", icon: "!", title: `주력이라고 적은 「${missingKey.keyword.replace(/^\S+\s/, "")}」가 네이버 플레이스에서 안 잡힙니다`, evidence: `「${missingKey.keyword}」 월 ${n(missingKey.volume)}회 · 우리 미노출`, action: missingKey.actions?.[0], rank: 85 });
  }
  // 플랫폼 불일치 × 실제 신환
  ...
  return out.sort((a, b) => b.rank - a.rank).slice(0, 7);
}
